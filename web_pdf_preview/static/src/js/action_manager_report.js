odoo.define('web.ReportPreview', function (require) {
"use strict";
...
ActionManager.include({
     ...
     */
    _triggerPreview: function (action, options, type){
        var self = this;
        var reportUrls = this._makeReportUrls(action);
        return this._previewReport(reportUrls[type]).then(function () {
            if (action.preview) {
                return self.window.open(reportUrls,'_blank','');
            } else {
        ...
        if (action.report_type === 'qweb-html') {
            ...
            return this.call('report', 'checkWkhtmltopdf').then(function (state) {
                ...
                if (state === 'upgrade' || state === 'ok') {
                    ...
                    return self._triggerPreview(action, options, 'pdf');
                } else {
