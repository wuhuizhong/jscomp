odoo.define('jscomp_customize.UserMenu', function (require) {
    "use strict";
    ...
    //避免错误，要再定义
    var documentation_url = 'http://www.odoo.com';
    var documentation_dev_url = 'http://www.odoo.com';
    var support_url = 'http://www.odoo.com';
    var account_title = 'My Account';
    var account_url = 'http://www.odoo.com';
    ...
    UserMenu.include({
        init: function () {
            ...
            self._rpc({
                ...
                _.each(res, function (lang) {
                    ...
                    lang_list += '<a role="menuitem" href="#" class="dropdown-item" data-lang-menu="lang" data-lang-id="' + lang['code']
                        + '"><img class="flag" src="jscomp_customize/static/src/img/flags/' + lang['code'] + '.png"/>' + lang['name'] + a + '</a>';
                });
            ...
            self._rpc({
                ...
                method: 'search_read',
                domain: [['key', '=like', 'jscomp_%']],
                fields: ['key', 'value'],
                lazy: false,
            }).then(function (res) {
                $.each(res, function (key, val) {
                    if (val.key == 'jscomp_documentation_url')
                        documentation_url = val.value;
                    if (val.key == 'jscomp_documentation_dev_url')
                        documentation_dev_url = val.value;
                    if (val.key == 'jscomp_support_url')
                        support_url = val.value;
                    if (val.key == 'jscomp_account_title')
                        account_title = val.value;
                    if (val.key == 'jscomp_account_url')
                        account_url = val.value;
                    //  控制显示
                    if (val.key == 'jscomp_show_lang' && val.value == "False") {
                        $('switch-lang').hide();
                    }
                    //注意， odoo12，主用户id=2, 加了个 __system__
                    if (session.user_context.uid > 2 || (val.key == 'jscomp_show_debug' && val.value == "False")) {
                        $('[data-menu="debug"]').hide();
                        $('[data-menu="debugassets"]').hide();
                        $('[data-menu="quitdebug"]').hide();
                    }
                    if (val.key == 'jscomp_show_documentation' && val.value == "False") {
                        $('[data-menu="documentation"]').hide();
                    }
                    if (val.key == 'jscomp_show_documentation_dev' && val.value == "False") {
                        $('[data-menu="documentation_dev"]').hide();
                    }
                    if (val.key == 'jscomp_show_support' && val.value == "False") {
                        $('[data-menu="support"]').hide();
                    }
                    if (val.key == 'jscomp_show_account' && val.value == "False") {
                        $('[data-menu="account"]').hide();
                    }
                    if (val.key == 'jscomp_account_title' && val.value) {
                        $('[data-menu="account"]').html(account_title);
                    }
                    if (val.key == 'jscomp_show_poweredby' && val.value == "False") {
